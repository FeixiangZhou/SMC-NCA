    ...
    def forward(self, model, valloader, device, outdir, wts=None):
        ...
        for i, item in enumerate(valloader):
            ...
            with torch.no_grad():
                ...
                features_last = features_last_list[0] # temporal representation from TCN
                # features_last2 = features_last_list[-1].permute(0, 2, 1)
                # features_last = features_last + features_last_list[-1].permute(0, 2, 1)# anchor
                # print(features_last.shape)


                # features_last = features_last / torch.norm(features_last, dim=1, keepdim=True)
                features = features_last.permute(0, 2, 1).cpu().numpy() #(2, 960, 1536)
                print(features.shape)

                for one_ele, count_ele, vid_id in zip(features, count, video_ids):
                    val_features = one_ele[:count_ele, :]
                    # print("val_features---", val_features[0,0:10])
