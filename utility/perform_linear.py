...
import glob
import os
# from utils_pdmb import get_all_scores
...
def get_gathered_features_labels_videoids(ground_truth_files_dir, dump_dir, label_name_to_label_id_dict, transposed, loadtext):
    ...
    print("ground_truth_files_dir---", ground_truth_files_dir)# mstcn_data/50salads/groundTruth/
    print("dump_dir---", dump_dir)#mstcn_data/50salads/results/ICC_SS_TAS/unsupervised_C2FTCN_split1_ts1/features_dump/


    features_arr_concat = []
    labels_arr_concat = []
    video_id_arr = []
    list_filenames = glob.glob(ground_truth_files_dir + "*.txt")#------------ori

    # list_testsplit1 = ['mstcn_data/50salads/groundTruth/rgb-01-1.txt', 'mstcn_data/50salads/groundTruth/rgb-01-2.txt',
    #      'mstcn_data/50salads/groundTruth/rgb-02-1.txt', 'mstcn_data/50salads/groundTruth/rgb-02-2.txt',
    #      'mstcn_data/50salads/groundTruth/rgb-03-1.txt', 'mstcn_data/50salads/groundTruth/rgb-03-2.txt',
    #      'mstcn_data/50salads/groundTruth/rgb-04-1.txt', 'mstcn_data/50salads/groundTruth/rgb-04-2.txt',
    #      'mstcn_data/50salads/groundTruth/rgb-05-1.txt', 'mstcn_data/50salads/groundTruth/rgb-05-2.txt',]
    # list_filenames = [i for i in list_filenames if i not in list_testsplit1]

    print("list_filenames---", len(list_filenames))  #len 40 ['mstcn_data/50salads/groundTruth/rgb-13-2.txt', 'mstcn_data/50salads/groundTruth/rgb-11-1.txt', ...]
    ...
    for i, filename in enumerate(list_filenames):
        ...
        # print("gd_truth_labels_for_video---", len (gd_truth_labels_for_video))
        ...
        if loadtext is True: 
            ...
            # numpy_data_arr = np.load(sample_numpy_data_arr).T #train i3d
            # print("numpy_data_arr---", numpy_data_arr.shape)
    ...
    print("features_arr_concat---", features_arr_concat.shape)  #(458692, 1536)
    print("labels_arr_concat---", labels_arr_concat.shape) #(458692,)
    print("video_id_arr---", video_id_arr.shape) #(458692,)
...
def perform_linear(feat_labels_videoids_dict, base_test_split_file, percentage_data, gapused, reg_v=1.0, no_split=False):
    ...
    '''
        final_result, result_arr = perform_linear(feat_labels_videoids_dict, base_test_split_file, percentage_data=100, gapused=chunk=20, 1, no_splits)
        args.base_test_split_file = args.base_dir + "splits/test.split{}.bundle"#--------------------------ori
    '''
    features_arr_concat = feat_labels_videoids_dict['features_arr_concat']
    labels_arr_concat = feat_labels_videoids_dict['labels_arr_concat']
    video_id_arr = feat_labels_videoids_dict['video_id_arr']

    print("features_arr_concat---", features_arr_concat.shape)  #(577595, 1536)
    print("labels_arr_concat---", labels_arr_concat.shape) #(577595,)
    print("video_id_arr---", video_id_arr.shape) #(577595,)
    ...
    if "50salads" in base_test_split_file:
        ...
        bg_list = ['background']
        datasetname = "gtea"
    elif "pdmb" in base_test_split_file:
        end = 3
        bg_list = ['other']
        # bg_list = []
        datasetname = "pdmb"
    ...
    for split in range(1, end):
        ...
        print("----------------------test_split_files------------------",test_split_files)
        split_train_features = []
        split_train_labels = []
        ...
        for feat, label, vid in zip(features_arr_concat, labels_arr_concat, video_id_arr):
            ...
            # get trainsplit1 and testsplit1
...
# ----------------------------------breakfast data---------------------------------------
        # test each activity in split1
        actl_labels = []
        actl_preds = []
        if action_dict_val_feat is not None and action_dict_val_label is not None \
           and action_dict_train_feat is not None and action_dict_train_label is not None:
            for action in action_dict_val_feat.keys(): #coffee' ,'cereals' , 'tea'...
...
#----------------------------------
        split_train_features = split_train_features[::gapused]
        split_val_features = split_val_features[::gapused]
        split_train_features = np.stack(split_train_features, axis=0)
        split_val_features = np.stack(split_val_features, axis=0)
        ...
        '''
        hape of train features  (22935, 1536)
        shape of val features  (5946, 1536)
        Shape of train labels  (22935,)
        Shape of val labels  (5946,)
        ...
        accuracy = accuracy_score(split_val_labels[:, None], preds)
        recall = recall_score(split_val_labels[:, None], preds, average="macro")
        recal_arr.append(recall * 100)
        cm = metrics.confusion_matrix(split_val_labels[:, None], preds)


        # calculate ave acc
        # for i in range(len(cm[0])):
        #     rowsum = np.sum(cm[i])
        #     print('aveacc_eachaction: %.3f' % (cm[i][i] / float(rowsum)))
        ...
        print(f"For split {split}, train acc = {train_score * 100.0}, val acc = {val_score * 100}, val recall = {recall * 100}.")
    
    print("All splits linear accuracy = ", np.mean(np.array(acc_arr)))
    print("All splits linear recall = ", np.mean(np.array(recal_arr)))
    ...
    return np.mean(np.array(acc_arr)), [np.mean(np.array(edit_scor_arr)), np.mean(np.array(f1_10_arr)), np.mean(np.array(f1_25_arr)), 
            np.mean(np.array(f1_50_arr)), acc_arr, edit_scor_arr, f1_10_arr, f1_25_arr, f1_50_arr, np.mean(np.array(recal_arr))]
...
def get_linear_acc_on_split(test_file, train_file, label_id_name_csv, feature_dump_dir, ground_truth_dir, percentage_data, 
    ...
    #test_file ----mstcn_data/50salads/splits/test.split1.bundle
    #train_file ---mstcn_data/50salads/splits/train.split1.bundle

    print("labels_name_id_dict---" ,labels_name_id_dict)


    feat_labels_videoids_dict = get_gathered_features_labels_videoids(ground_truth_dir, feature_dump_dir, 
                                                                      labels_name_id_dict['label_name_to_label_id_dict'], transposed, loadtxt)

    # print("feat_labels_videoids_dict---" ,feat_labels_videoids_dict)
