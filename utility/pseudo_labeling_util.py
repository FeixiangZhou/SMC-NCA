...
def pseudo_labeling(args, data_loader, model, itr):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, indexs, _) in enumerate(data_loader):
            ...
            out_prob = torch.stack(out_prob)   #[10, 128, 10]
            # print("out_prob-----", out_prob.shape)
            out_prob_nl = torch.stack(out_prob_nl)

            out_std = torch.std(out_prob, dim=0) #[128,10]
            out_std_nl = torch.std(out_prob_nl, dim=0)

            out_prob = torch.mean(out_prob, dim=0) #[128,10]
            out_prob_nl = torch.mean(out_prob_nl, dim=0) #[128,10]

            max_value, max_idx = torch.max(out_prob, dim=1)#[128], [128]

            max_std = out_std.gather(1, max_idx.view(-1,1)) #[128, 1]
            out_std_nl = out_std_nl.cpu().numpy()
            
            #selecting negative pseudo-labels
            interm_nl_mask = ((out_std_nl < args.kappa_n) * (out_prob_nl.cpu().numpy() < args.tau_n)) *1 #[128,10] each row, multiple 1
            # print("interm_nl_mask-----", interm_nl_mask)

            #manually setting the argmax value to zero
            for enum, item in enumerate(max_idx.cpu().numpy()):
                interm_nl_mask[enum, item] = 0
            nl_mask.extend(interm_nl_mask)  #[128*n, 10]


            idx_list.extend(indexs.numpy().tolist()) #[128* n]
            gt_list.extend(targets.cpu().numpy().tolist())
            target_list.extend(max_idx.cpu().numpy().tolist())

            #selecting positive pseudo-labels
            if not args.no_uncertainty:
                selected_idx = (max_value>=args.tau_p) * (max_std.squeeze(1) < args.kappa_p) #[128] #[False, false---]
                # print("selected_idx-----", selected_idx)

            else:
                selected_idx = max_value>=args.tau_p

            tes = max_idx[selected_idx].cpu().numpy().tolist()
            print("tes-----", len(tes))  #<128
            #select samples
            pseudo_maxstd.extend(max_std.squeeze(1)[selected_idx].cpu().numpy().tolist()) # no false, only leave true
    ...
    for i in range(len(idx_list)): #all unlabelled train samples
        if idx_list[i] not in pseudo_idx and sum(nl_mask[i]) > 0: #have negnitive label
            pseudo_nl_mask.append(nl_mask[i])
            pseudo_nl_idx.append(idx_list[i]) # len 31499
            nl_gt_list.append(gt_list[i])

    nl_gt_list = np.array(nl_gt_list)
    pseudo_nl_mask = np.array(pseudo_nl_mask) #pseudo_nl_mask----- (31499, 10)
    print("pseudo_nl_mask-----", pseudo_nl_mask.shape)
    one_hot_targets = np.eye(args.num_classes)[nl_gt_list] #----- (31499, 10)
    ...
    flat_one_hot_targets = flat_one_hot_targets[np.where(flat_pseudo_nl_mask == 1)] #choose negative PL
