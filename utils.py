...
def calculate_mof(ground_truth_path_name, prediction_path, background_class):
    ...
    # loop over all recognition files and evaluate the frame
    list_allvideo = []
    list_allcorrect = []
    list_combine_allvideo = []
    list_combine_allcorrect = []

    for filename in filelist:
        # correct, frames, tp_arr, fp_arr, fn_arr, edit_score_value = recog_file(filename, ground_truth_path_name,overlap, background_class)
        correct, frames, tp_arr, fp_arr, fn_arr, edit_score_value = recog_file(filename, ground_truth_path_name,overlap, background_class)
    ...
    # acc_eachclass = np.true_divide(list_combine_allcorrect, list_combine_allvideo)
    # print('recall for each class:', acc_eachclass)
    #
    # acclist = []
    # for k in range(9):
...
def get_all_scores(pred, label, bg_class_list):
    ...
    for overlap in [0.1, 0.25, 0.5]:
        ...
        # tp, fp, fn = f_score(pred, label, overlap=overlap, bg_class=bg_class_list)
